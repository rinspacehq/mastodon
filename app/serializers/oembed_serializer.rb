  ...
  def html
    <<~HTML.squish
      <blockquote class="mastodon-embed" style="#{INLINE_STYLES[:blockquote]}">
        <a href="#{ActivityPub::TagManager.instance.url_for(object)}" target="_blank" rel="noopener noreferrer" style="#{INLINE_STYLES[:status_link]}">
          ...
          <div style="#{INLINE_STYLES[:div_view]}">View on Rinspace</div>
        </a>
      </blockquote>
    HTML
  end
