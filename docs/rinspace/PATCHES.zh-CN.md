...
> 权威关系更新（2026-09-06）：私有 `rinspace` 是产品与集成契约权威。本 fork 只对
> Mastodon 所属的里世界代码和本地社交数据负责。下文涉及 `@rinspace/world-shell`、公开
> 世界路由契约或工作树 release 的内容记录的是已废弃公开前端实验，不得用于新发布。
...
该表保存 2026-09-05 的最初核查，不代表当前 fork 提交。补丁编号仍用于升级审计，但每个新发布必须记录实际不可变提交，并拒绝 dirty 工作树。
...
| `RIN-003` | `rinspace-product`  | 同上                                       | Mastodon 所属导航 adapter、里世界布局和品牌适配，对齐私仓 `rinspace/ui` 导航；每个控件保留原生搜索、通知、会话、发布与权限行为 | `yarn typecheck`、聚焦 adapter 测试、生产 Vite 构建及与私仓批准基准的视觉比较 | fork 变更进入公开 AGPL 对应源码；发布不再包含公开前端包            | 移除废弃 `world-shell` import 时不得减少按钮或制造假状态，并比较两个 runtime                |
| `RIN-004` | `long-lived-safety` | 同上                                       | 客户端路由规范化、路由清单、PWA 启动地址、Service Worker 缓存与 push 落点                                                      | `yarn audit:rinspace-routes`；Service Worker 与路径聚焦 Vitest                | 改变 PWA 导航及缓存行为                                            | 新增上游路由必须先由私仓 Rinspace 契约分类，否则审计失败                                    |
| `RIN-005` | `long-lived-safety` | 同上                                       | 预配 OIDC 身份、签名身份/标签/关注 API、资料与 handle 生命周期、HTTPS 精确主机头像/封面导入，支持可选更新与显式清除            | binding、服务签名、OIDC 和领域服务聚焦测试                                    | 新增私有集成 API 与 `RINSPACE_PROFILE_MEDIA_HOSTS`；密钥仅留服务端 | 保留冲突 fail closed、删除终态与头像/封面字段缺失兼容语义；媒体白名单不得扩张成联邦出站能力 |
...
4. 按私有 `rinspace` 拥有的版本化世界路由契约重新枚举 Rails、React Router、API、OAuth、streaming、媒体、ActivityPub、WebFinger、NodeInfo、Sidekiq delivery 和 Service Worker 路由。
...
9. 构建不可变镜像与源码制品，记录 digest、准确 fork 提交、私仓 Rinspace 集成 revision 和生成源码 provenance，并在批准发布前演练回滚。不得重新创建包含 `rinspace-web` 或 `@rinspace/world-shell` 的包锁。
...
- fork 实现私有 `rinspace` 拥有的准确版本集成契约；release 不消费 `rinspace-web`，不复制未经评审的前端工作树，也不使用浮动分支或 tag。
