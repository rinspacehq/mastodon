...
> Authority update (2026-09-06): private `rinspace` is the product and integration-contract
> authority. This fork is authoritative only for Mastodon-owned inner-runtime code and local social
> data. References below to `@rinspace/world-shell`, a public world-route contract or a working-tree
> release describe the retired public-frontend experiment and must not be used for a new release.
...
This table records the original 2026-09-05 inspection and is not the current fork commit. The patch IDs remain useful upgrade categories, but every new release must record its actual immutable commit and must not publish a dirty tree.
...
| `RIN-003` | `rinspace-product`  | same                                       | Mastodon-owned navigation adapter, inner layout and branding matched to the private `rinspace/ui` navigation; each control retains native search, notification, session, publish and permission behavior | `yarn typecheck`, focused adapter tests, production Vite build and visual comparison against the approved private reference | Fork changes remain in the published AGPL Corresponding Source; no public-frontend package is a release input | Replace the retired `world-shell` import without reducing controls or creating fake state; compare both runtimes visually                                     |
| `RIN-004` | `long-lived-safety` | same                                       | Client route canonicalization, route inventory, PWA start URL, Service Worker cache and push targets                                                                                                     | `yarn audit:rinspace-routes`; focused Vitest Service Worker and path suites                                                 | PWA navigation/cache behavior changes                                                                         | New upstream routes fail the explicit audit until classified by the private Rinspace contract                                                                 |
| `RIN-005` | `long-lived-safety` | same                                       | Pre-provisioned OIDC identity, signed identity/tag/follow APIs, profile and handle lifecycle, exact-host HTTPS avatar/header import with optional-update and explicit-clear semantics                    | Focused binding, service-request, OIDC and service specs                                                                    | New private integration API and `RINSPACE_PROFILE_MEDIA_HOSTS`; keys remain server-only                       | Preserve fail-closed collision, terminal deletion and absent-avatar/header compatibility semantics; do not broaden the media allowlist into federation egress |
...
4. Re-enumerate Rails, React Router, API, OAuth, streaming, media, ActivityPub, WebFinger, NodeInfo, Sidekiq delivery, and service-worker routes against the versioned world-route contract owned by private `rinspace`.
...
9. Build immutable images and source artifacts, record their digests with the exact fork commit, private Rinspace integration revision and generated-source provenance, and rehearse rollback before release approval. Do not recreate a `rinspace-web` or `@rinspace/world-shell` package lock.
...
- The fork implements the exact versioned integration contract owned by private `rinspace`. It does not consume `rinspace-web`, copy an unreviewed frontend worktree, or use a floating branch/tag in a release.
