...
| `RIN-005` | `long-lived-safety` | same | Pre-provisioned OIDC identity, signed identity/tag/follow APIs, profile and handle lifecycle, exact-host HTTPS profile-media import | Focused binding, service-request, OIDC and service specs | New private integration API and `RINSPACE_PROFILE_MEDIA_HOSTS`; keys remain server-only | Preserve fail-closed collision and terminal deletion semantics; do not broaden the media allowlist into federation egress |
